...
import { Upload, Save, Loader2, Building2, Plus, Search } from "lucide-react";
...
export function EmpresasCadastro() {
  ...
  const [ultimoCnpj, setUltimoCnpj] = useState("");
  ...
  async function preencherPeloCnpj(valor?: string) {
    ...
    try {
      ...
      setForm((f) => ({
        ...
        nome_fantasia: f.nome_fantasia || d.nome_fantasia || d.razao_social || "",
      ...
      toast.success("Dados encontrados na Receita Federal", {
        description: d.situacao ? `Situação: ${d.situacao}` : undefined,
      });
    } catch (e) {
      toast.error("Não foi possível consultar o CNPJ", { description: (e as Error).message });
    } finally {
      setBuscando(false);
    }
  }

  function onCnpjChange(valor: string) {
    const d = soDigitos(valor);
    setForm((f) => ({ ...f, cnpj: mascaraCnpj(valor) }));
    if (isAdmin && d.length === 14 && d !== ultimoCnpj) {
      setUltimoCnpj(d);
      void preencherPeloCnpj(d);
    }
  }
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      if (form.emitente_padrao) {
        await supabase.from("company_settings").update({ emitente_padrao: false }).eq("emitente_padrao", true);
      }
      if (novaEmpresa) {
        const { data, error } = await supabase.from("company_settings").insert(form).select("id").single();
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>
              Digite o CNPJ para buscar os dados na Receita Federal e preencher tudo automaticamente
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="rounded-lg border bg-muted/30 p-3">
              ...
              <div className="mt-1.5 flex gap-2">
                ...
                <Button
                  ...
                  disabled={!isAdmin || buscando}
                >
                  {buscando ? <Loader2 className="mr-2 size-4 animate-spin" /> : <Search className="mr-2 size-4" />}
                  Buscar dados
                </Button>
              </div>
              <p className="mt-2 text-xs text-muted-foreground">
                Razão social, endereço, cidade, UF, CEP, telefone e e-mail são preenchidos sozinhos. Inscrição estadual
                e RNTRC precisam ser digitados.
              </p>
            </div>
